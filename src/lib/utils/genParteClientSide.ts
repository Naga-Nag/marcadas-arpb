...
import { getfechaMarcada } from '$lib/stores/global';
import ExcelJS from 'exceljs';

export async function generateExcelFromTemplate(marcadas: Array<Marcada>) {
     ...
     marcadas.forEach((record: any, index: number) => {
          ...
          row.getCell(1).value = record.Nombre;
          row.getCell(2).value = record.MR;
          row.getCell(3).value = record.CUIL;
     ...
     link.download = `Parte ${Departamento} ${getfechaMarcada()}.xlsx`;
     link.click();
